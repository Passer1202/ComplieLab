...
static CPValue meetValue(CPValue v1, CPValue v2) {
    ...
    //3. 一个是UNDEF, 另一个是CONST, 返回CONST
    if(v1.kind == UNDEF) return get_CONST(v2.const_val);
    if(v2.kind == UNDEF) return get_CONST(v1.const_val);
    //4. 都是CONST, 返回相等的CONST, 否则返回NAC
    assert(v1.kind == CONST && v2.kind == CONST);
...
ConstantPropagation_isForward (ConstantPropagation *t) {
    // TODO: return isForward?; //forward
    return true; // Constant Propagation is a forward analysis
}
